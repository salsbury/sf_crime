...
shinyServer(function(input, output){
  zip_df <- reactive({
        sfn[sfn$ZIP_CODE == input$zipcode,]
              })
  clean_tc_df <- reactive({
  ...
  zipcode_name <- reactive({
                    switch(input$zipcode,
  ...
  output$info <- renderPrint({
    new_df <- nearPoints(clean_tc_df(), input$plotclick, xvar = "X", yvar = "Y")
    unique(new_df$Address)
  })
